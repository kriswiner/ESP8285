/* EM7180_MPU9250_MS5637_t3 Basic Example Code
 ...
 This sketch uses SDA/SCL on pins 0/2 of the ESP8266/85, respectively, and it uses the Wire library Wire.h.
 
 SDA and SCL have external 4K7 pull-up resistors (to 3.3V).

 This setup is for the ESP8285 Development board
 Hardware setup:
 EM7180 Mini Add-On ------- ESP8285
 VDD ---------------------- 3.3V
 SDA ----------------------- 0
 SCL ----------------------- 2
 GND ---------------------- GND
 INT------------------------ 9 (or any GpiO pin)
...
// pin definitions
int intpin = 14;  // This can be changed to any available GpiO
bool newData = false;
bool newMagData = false;
int myLed  = 15; // LED on the ESP8285 Development Board
...
float Q[4] = {0, 0, 0, 0}; // hardware quaternion data register
...
float a12, a22, a31, a32, a33;            // rotation matrix coefficients for Software Euler angles and gravity components
...
float lin_ax, lin_ay, lin_az;             // Software linear acceleration (acceleration with gravity component subtracted)
float lin_Ax, lin_Ay, lin_Az;             // Hardware linear acceleration (acceleration with gravity component subtracted)
float q[4] = {1.0f, 0.0f, 0.0f, 0.0f};    // vector to hold software quaternion
...
void setup()
{
  WiFi.mode(WIFI_OFF); //WIFI_AP, WIFI_STA, WIFI_AP_STA or WIFI_OFF // turn off wifi to save power
